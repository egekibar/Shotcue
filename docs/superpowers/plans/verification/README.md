...
| V2 | İzin onboarding'i + relaunch | kısmen (ajan) | Ekran Kaydı izni olmayan build açılışta onboarding'i açtı: bir Shotcue penceresi 480 × 539, `lsappinfo` → `Foreground` (Dock ikonu). Relaunch yolu probe ile (N1 düzeltmesinden önceki kodda; "Başla" o zaman pencereyi kapatıp doğrudan `NSApp.terminate` çağırıyordu): pid 65763 `relaunching after the Screen Recording grant` → eski süreç bitti → yeni pid 65800 0,2 sn sonra `hotkey ⌃⇧2 registered` (çakışma yok), ikinci relaunch yok. **N1'den beri akış:** "Başla" → log `quitting to relaunch after the Screen Recording grant` → çıkış her çıkış gibi `TerminationController`'ın kapanışından geçer (çalışan run varsa yeniden başlatma diyaloğu, bkz. R1) ve onboarding penceresi çıkış gerçekleşene kadar açık kalır → `applicationWillTerminate`'te `relaunching after the Screen Recording grant` (yeniden başlatıcı ancak çıkış kesinleşince başlar) → süreç biter, yeni PID. PENDING (user): `make reset-tcc`, satırlar "Sorulmadı" + "İzin ver" / reddedilince "Reddedildi" + "Sistem Ayarları", izni ver, **"Başla"** → (çalışan run yokken diyalog çıkmaz) yeni PID, onboarding bir daha açılmaz; `v2-onboarding.png`. **Yaz:** izin verildikten sonra macOS "yeniden başlat" diyaloğu gösterirse **Later**'ı seç, onboarding'e dönüp **"Durumları yenile"**'ye bas ve "Başla"nın yeniden başlatmadan etkinleşip etkinleşmediğini not et (`CGPreflightScreenCaptureAccess` süreç yeniden başlamadan `true` dönmeyebilir; dönmüyorsa kullanıcı "Başla"ya basamaz ve uygulamayı elle kapatıp açması gerekir — bulguyu buraya yaz). |
...
| V6 | Gönder → canlı log → done → bildirim → terminal | PENDING (user) | Gerçek `claude -p` gerekir. `select state, subtype, num_turns, cost_usd, git_head_before is not null, git_head_after is not null from run order by started_at desc limit 1;` → `succeeded\|success\|n\|usd\|1\|1`; bir Analiz run'ı sonrası `git status --porcelain` boş; `v6-live-log.png`. "Done" bildiriminin gövdesi özet + maliyet taşır: sonucun ilk satırı (yoksa "Tamamlandı") ve ` · $0.41` (spec §5.5; Core değişikliği gerekmedi — gövde metni `RunCoordinator`'da kuruluyor, son düzeltme dalgası M1); maliyeti olmayan sonuçta yalnızca özet. "Terminalde devam et" açılan terminal proje klasöründe başlar (`.command` önce `cd -- '<proje>'`, son düzeltme dalgası M6): `pwd` proje yolunu gösterir ve `claude --resume <id>` oturumu bulur ("No conversation found" görülürse not et). |
...
| V11 | Yarım kalan run → failed(interrupted) | PENDING (user) | Gerçek run gerekir: run başlayınca `pkill -9 -x Shotcue` → `running`; yeniden açınca `select state, error from run order by started_at desc limit 1;` → `failed` + `interrupted`; "Terminalde devam et" çalışır. Terminal proje klasöründe açılır (M6) — yarıda kalan oturum devam ettirilebiliyor mu? (Oturum hiç başlamadıysa, örn. `starting`'de kesildiyse, düğme pasiftir: M4.) |
...
| `error_max_turns` | Max tur = 1, bir task gönder | Run `failed`, otomatik yeniden denenmez; `run.error` = `error_max_turns` (kod), Inspector run satırında Türkçe "Tur limiti aşıldı (1 tur)." + öneri "Ayarlar > Claude'dan tur limitini artırıp yeniden çalıştır." (I6); RUN_FAILED bildirimi aynı metni taşır | PENDING (user) |
| Zaman aşımı | Zaman aşımı = 1 dk, uzun bir görev | Run `failed`, `run.error` = `timeout`; Inspector "Zaman aşımı: çalışma süre sınırında durduruldu." (I6), süreç SIGINT → SIGKILL | PENDING (user) |
| Proje klasörü yok | Proje yolunu var olmayan bir dizine değiştir | Run başlamaz, proje kırmızı; `run.error` = `project_folder_missing: <yol>`, Inspector "Proje klasörü bulunamadı." + düzeltme önerisi, yol altta (I6) | PENDING (user) |
...
| E3 | Gerçek CLI ile iptal ve limit | Çalışan run'da Inspector > İptal → run `cancelled` (Inspector: "İptal edildi."); max tur = 1 → run satırı "Tur limiti aşıldı" + limit artırma önerisi (I6) | PENDING (user) |
...
| F3 | Model yokken sesli not → indirme teklifi (C1 (c), spec §6.2) | Model indirilmemişken hızlı panelde kayıt yap → panelde "Model indirilmedi; kayıt saklandı…" + **"Modeli indir (≈1,6 GB)"** düğmesi (hiçbir indirme kendiliğinden başlamaz; `ls ~/Library/Application\ Support/Shotcue/models` boş kalır). Inspector'da not "çevriliyor" değil **"model indirilmedi"** der, yanında aynı düğme. Düğmeye bas (kullanıcı onayı = indirme) → panel/Inspector/Ayarlar > Ses aynı ilerlemeyi gösterir (%n), ikinci basış ikinci indirme başlatmaz; bitince bekleyen not yazıya dökülür (`transcript_state` → `done`). | PENDING (user) |
| F4 | Kuyruk yeniden başlatmadan sonra devam eder (I1) | Eş zamanlı çalışma = 1, aynı ya da farklı projede iki görev gönder (biri `running`, biri `queued`); uygulamayı kapat (ilk run iptal edilir, I2) ve aç → `queued` görev **kendiliğinden** başlar (başka bir gönderim ya da Duraklat/Sürdür gerekmez). `claude` bulunamazken açılırsa kuyruktaki görevler `queued` kalır, "not found" hatasıyla `failed` olmaz. | PENDING (user) |
| F5 | Çalışan run varken çıkış (I2) | Bir görev `running` iken menüden **Çık** (ya da ⌘Q) → "Çalışan 1 görev durdurulacak." diyaloğu, **"Durdur ve çık"** / **"Vazgeç"** (Esc = Vazgeç). Vazgeç → uygulama ve run sürer. Durdur ve çık → ≤ ~12 sn içinde kapanır; `select state, error from run order by started_at desc limit 1;` → `cancelled`, `ps aux \| grep "claude -p"` boş (yetim `claude` yok). Hızlı panele not yazılıyken / ses kaydı sürerken çıkış → not projesiz inbox taslağı olarak kalır, kayıt `voice_note` satırı + çalınabilen `.m4a` olur; Inspector'da odak kaybetmemiş not taslağı kaydedilir. | kısmen (ajan): probe — sahte `claude` (`fake-claude.sh`, `hang`) ile gerçek uygulama: SIGTERM → 0,17 sn'de "quit: every run stopped", süreç çıktı; run `cancelled`, görev `cancelled`; hızlı paneldeki yazılmış not projesiz inbox taslağı (`probe panel note…`), Inspector taslağı kaydedildi. SIGINT'i yok sayan bir `claude` yerine geçen betikle: runner 10 sn sonra SIGKILL, "every run stopped" +11,2 sn. AppKit çıkış yolu (run yokken, bir run-loop zamanlayıcısından `terminate`): `.terminateLater` sırasında kayıt işi çalıştı, not ve taslak kaydedildi, süreç çıktı. Probe verisi geçici bir depolama kökündeydi (gerçek veritabanına dokunulmadı), probe kodu silindi. PENDING (user): diyalog, ses kaydı (mikrofon izni), gerçek `claude`. |
| F7 | Git güvenlik ağı kapalı başarısız olur (I4) | Proje ayarlarında "Her çalıştırmayı yeni branch'te başlat" açık, proje klasörü git deposu **değil** (ya da çakışan bir durum) → gönder: run `failed`, `error` `git_branch_failed: fatal: …`, görev `ready`, RUN_FAILED bildirimi Türkçe ("Yeni git branch'i açılamadı; claude başlatılmadı…"), `claude -p` hiç başlamaz. Aynı görevi git deposu olan projede iki kez çalıştır → iki ayrı dal: `git branch --list 'shotcue/*'` → `shotcue/<görev8>-<run8>` biçiminde iki satır. "Çalıştırmadan önce değişiklikleri stash'le" açıkken stash başarısız olursa aynı davranış (`git_stash_failed`). | PENDING (user) |
| F8 | Terminalde devam et — proje klasöründen (M4, M6) | Bir run bittikten sonra Inspector'dan ve RUN_DONE bildiriminden "Terminalde devam et" → Terminal açılır, `pwd` görevin proje klasörü, `claude --resume <id>` oturumu açar. Yalnızca başlamadan reddedilmiş run'ı olan görevde (örn. model yokken sesli not, F2) "Terminalde devam et" / "Desktop'ta aç" **pasif**. Boşluk ve `'` içeren bir proje yolu da çalışır. | PENDING (user) |
| F6 | `make install` / `pkill` çalışan bir run sırasında (I2) | Bir run sürerken `make install` → "Stopped the running Shotcue after N s" (N ≤ ~12), ardından "Installed:"; yeni uygulama açılınca run `cancelled` (yarıda kalanlar `interrupted` değil), `claude -p` süreci kalmaz. | kısmen (ajan): probe — `scripts/install.sh` çalışan probe uygulamasına SIGTERM gönderdi, çıkışı bekledi ("after 0 s"; SIGINT'i yok sayan betikle 11 sn), sonra kurdu; run `cancelled`. Gerçek `claude` PENDING (user). |

## Yeniden inceleme düzeltmeleri (N1 ve küçük bulgular, v1.0.0 öncesi)

Kod testleri ajan tarafından koşuldu; çıkış yolları ayrıca `.terminateLater` yanıtlayan başsız bir karalama AppKit
uygulamasıyla ölçüldü (eski çağrı biçimi dondu, yenisi kapandı; kapanış sürerken gelen ikinci çıkış isteği yok sayıldı).
Aşağıdaki satırlar gerçek uygulamada kullanıcının göreceği davranışı kontrol eder.
...
- **Bildirimde maliyet (kapandı, son düzeltme dalgası M1):** "done" bildirimi başlık + sonucun ilk satırı + maliyeti
  taşır ("Özet satırı · $0.42"). Önceki notun aksine Core değişikliği gerekmiyordu: gövde bir metin, `RunCoordinator`
  onu sonucun `total_cost_usd`'siyle kuruyor (V6).
...
- **Terminalde devam et** (son düzeltme dalgası M6): `.command` dosyası önce görevin proje klasörüne geçer
  (`cd -- '<proje>' || exit 1`, tek tırnaklı), sonra `'<claude yolu>' --resume <uuid>` çalıştırır; oturum id'si UUID
  değilse dosya hiç yazılmaz. Claude Code oturumları proje klasörüne göre sakladığı ve devam eden oturumun araçları
  orada çalışması gerektiği için. Görevin projesi yoksa düğme hata verir (V6, V11, P7, F8).
